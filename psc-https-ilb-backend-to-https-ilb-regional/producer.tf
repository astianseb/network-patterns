...
resource "google_compute_forwarding_rule" "app_forwarding_rule" {
  ...
  port_range            = "443"
  target                = google_compute_region_target_https_proxy.producer.id
...
# http proxy
resource "google_compute_region_target_https_proxy" "producer" {
  name     = "l7-ilb-target-http-proxy"
  provider = google-beta
  region   = var.region
  project  = google_project.producer.project_id
  url_map  = google_compute_region_url_map.producer.id
