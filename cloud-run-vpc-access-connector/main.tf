...
resource "google_compute_subnetwork" "vpc_direct_egress" {
  name     = "vpc-direct-egress"
  project  = google_project.project.project_id

  ip_cidr_range = "10.101.0.0/26"
  region        = var.region
  network       = google_compute_network.network["net-a"].id
}

resource "google_compute_firewall" "allow_vde" {
  project   = google_project.project.project_id

  name      = "allow-vde"
  network   = google_compute_network.network["net-a"].name
  ...
  source_ranges = [google_compute_subnetwork.vpc_direct_egress.ip_cidr_range]
}


#####

resource "google_compute_subnetwork" "vpc_access_connector" {
...
resource "google_compute_firewall" "allow_vac" {
  project   = google_project.project.project_id
  ...
  source_ranges = [google_compute_subnetwork.vpc_access_connector.ip_cidr_range]
}

resource "google_vpc_access_connector" "default" {
  name     = "sg-access-connector"
  project  = google_project.project.project_id
  
  region = var.region
  subnet {
    name = google_compute_subnetwork.vpc_access_connector.name
  }
  machine_type = "e2-standard-4"
}

resource "google_cloud_run_service" "default" {
  name     = "sg-looking-glass"
  project  = google_project.project.project_id
  ...
  template {
    metadata {
      annotations = {
        "run.googleapis.com/vpc-access-connector" = "sg-access-connector"
        "run.googleapis.com/vpc-access-egress" = "private-ranges-only"
      }
    }
    spec {
      containers {
        image = "astianseb/sg-looking-glass"
        ports {
          container_port = 8000
        }
      }
